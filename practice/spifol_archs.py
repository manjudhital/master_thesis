...
def complex_adam(step_size, b1=0.9, b2=0.999, eps=1e-8):
    ...
    def update(i, g, state):
        ...
        vhat = v / (1 - b2 ** (i + 1))
        #print(f'COMPLEX ADAm update vhat type ={type(vhat)}')  # To check if it's a JAX array
        #print(f'COMPLEX ADAm ={(step_size)}') 
        step_size_val = step_size(i)
        x = (x - step_size_val * mhat) / (jnp.sqrt(vhat) + eps) 
        return x, m, v
